...
def students_and_examinations(students: pd.DataFrame,
    ...
    # all student-subject pairs
    full = students.merge(subjects, how="cross")
    ...
    res = full.merge(counts, on=["student_id", "subject_name"], how="left")
    res["attended_exams"] = res["attended_exams"].fillna(0).astype(int)

    # sort output
    res = res.sort_values(["student_id", "subject_name"])
