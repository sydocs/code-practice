...
SELECT 
    ...
    s.store_name,
    s.location,
    maxp.product_name AS most_exp_product,
    minp.product_name AS cheapest_product,
    ROUND(minp.quantity * 1.0 / maxp.quantity, 2) AS imbalance_ratio
FROM store_counts sc
JOIN stores s 
    ON sc.store_id = s.store_id
JOIN max_price_product maxp 
    ON sc.store_id = maxp.store_id
JOIN min_price_product minp 
    ON sc.store_id = minp.store_id
WHERE sc.product_cnt >= 3
  AND maxp.quantity < minp.quantity
