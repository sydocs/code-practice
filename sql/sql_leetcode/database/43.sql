...
SELECT
    ...
    cs.monthly_amount AS current_monthly_amount,
    ub.max_historical_amount,
    DATEDIFF(ub.last_date, ub.first_date) AS days_as_subscriber
FROM user_base ub
JOIN current_state cs
    ON ub.user_id = cs.user_id
WHERE
    cs.event_type <> 'cancel'
    AND ub.downgrade_count >= 1
    AND ub.max_historical_amount > 0
    AND cs.monthly_amount < 0.5 * ub.max_historical_amount
    AND DATEDIFF(ub.last_date, ub.first_date) >= 60
ORDER BY
